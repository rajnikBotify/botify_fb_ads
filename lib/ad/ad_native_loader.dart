...
class FacebookNativeAd extends StatefulWidget {
  ...
  final void Function(FacebookNativeAdResult, dynamic)? listener;
  final FacebookNativeAdType adType;
  final double width;
  final double height;
...
class _FacebookNativeAdState extends State<FacebookNativeAd>
  ...
  _getAdsType(FacebookNativeAdType type) {
    if (type == FacebookNativeAdType.NATIVE_AD_BIG) {
      return NATIVE_AD_BIG;
    } else if (type == FacebookNativeAdType.NATIVE_AD_SMALL) {
      return NATIVE_AD_SMALL;
    } else if (type == FacebookNativeAdType.NATIVE_AD_BIG_ROW) {
      return NATIVE_AD_BIG_ROW;
    } else if (type == FacebookNativeAdType.NATIVE_AD_FULL_SCREEN) {
      ...
      switch (call.method) {
        case ERROR_METHOD:
          if (widget.listener != null) {
            widget.listener!(FacebookNativeAdResult.ERROR, call.arguments);
            if (!isAdError) {
              setState(() {
                isAdError = true;
              });
        ...
        case LOADED_METHOD:
          if (widget.listener != null) {
            widget.listener!(FacebookNativeAdResult.LOADED, call.arguments);
          }
          if (!isAdReady) {
            setState(() {
              isAdReady = true;
        ...
        case CLICKED_METHOD:
          if (widget.listener != null) {
            widget.listener!(FacebookNativeAdResult.CLICKED, call.arguments);
          }
          break;
        case LOGGING_IMPRESSION_METHOD:
          if (widget.listener != null) {
            widget.listener!(
                FacebookNativeAdResult.LOGGING_IMPRESSION, call.arguments);
          }
          break;
        case MEDIA_DOWNLOADED_METHOD:
          if (widget.listener != null) {
            widget.listener!(FacebookNativeAdResult.MEDIA_DOWNLOADED, call.arguments);
          }
          break;
      }
